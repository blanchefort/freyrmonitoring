...
Rake = RAKE.Rake(os.path.join(settings.ML_MODELS, 'stopwords.txt'))
...
def generate_kw(title, text, threshold):
...
def generate_corpus_embs(titles, texts):
    ...
    for title, text in zip(titles, texts):
        text = text.replace('\n', ' ')
        text = re.sub(r'http\S+', '', text.strip(), flags=re.MULTILINE)
        lag_kwds.append(generate_kw(title.replace('\xa0', ' '), text, 4.5))
    corpus_embeddings = embedder.encode(lag_kwds)
    return corpus_embeddings

def article_similars(embeddings):
...
def select_maximal_cluster(items):
...
def create_cluster(texts, titles):
    ...
    for text, title in zip(texts, titles):
        full_text += generate_kw(title, text, 6) + ' '
    cluster_embedding = embedder.encode(full_text, convert_to_tensor=True)
    
    item_kwds = []
    for text, title in zip(texts, titles):
        item_kwds.append(generate_kw(title, text, 4.5))
    item_embs = embedder.encode(item_kwds, convert_to_tensor=True)
...
def select_cluster_articles(cluster_embedding, corpus_embeddings):
...
def rearrange_list(lst, to_remove):
...
def get_system_ids(lag_idx, reltive_ids):
...
def get_nonclustered_data(lag_idx, lag_titles, lag_texts, clustered_ids):
...
def generate_new_clusters(corpus_embeddings, lag_titles, lag_texts):
    '''Выявляем кластеры, пока есть статьи-кандидаты
    '''
    similars = article_similars(embeddings=corpus_embeddings)
    candidates = 0
    ...
    while candidates > 0:
        # находим самый большой кластер
        idx, items_ids = select_maximal_cluster(similars)
        ...
        top_idx, cluster_embedding, cluster_kwds = create_cluster(items_texts, items_titles)
        if len(items_titles[top_idx]) < 1:
            cluster_title = generate_title(items_texts[top_idx])
        else:
            cluster_title = items_titles[top_idx]

        # Проходим по всему корпусу, и выбираем статьи, семантически похожие на кластер
        selected, max_id, max_score = select_cluster_articles(cluster_embedding, corpus_embeddings)
        if (top_idx != max_id) and max_score > .7:
            if len(lag_titles[max_id]) < 1:
                cluster_title = generate_title(lag_titles[max_id])
            else:
        ...
        similars = rearrange_list(similars, selected)
